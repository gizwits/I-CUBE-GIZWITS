...
char  mqtt_num_rem_len_bytes_safe( const uint8_t* buf,const uint8_t buflen );
...
typedef void (*TcpSendCb)(int fd, int result, void *arg);
typedef int (*mqttSendFun)(int socket_info, uint8_t* buf, unsigned int count,TcpSendCb cb,void* arg );

typedef struct {
    int socketid;
    // int (*mqttsend)(int socket_info, uint8_t* buf, unsigned int count,TcpSendCb,void* arg );
    mqttSendFun mqttsend;
    // Connection info
    char clientid[50];
    // Auth fields
...
int mqtt_publish(mqtt_broker_handle_t* broker, const char* topic, const char* msg,const uint16_t msglen, uint8_t retain);
...
int mqtt_publish_with_qos(mqtt_broker_handle_t* broker, const char* topic, const char* msg, const uint16_t msglen,uint8_t retain, uint8_t qos, uint16_t* message_id,TcpSendCb fun,void *arg );
...
char mqtt_len_of_bytes( int len, uint8_t *buf );
